 ...
 */
public class MachinePistolCustomModel implements IOverrideModel
{
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        BakedModel bakedModel = ExpandedModelComponents.MACHINE_PISTOL_BASE.getModel();
		...
		{
            RenderUtil.renderModel(ExpandedModelComponents.MACHINE_PISTOL_SIGHTS.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
		}
        ...
        if(isPlayer && correctContext)
        {
            ...
            boltTranslations = boltTranslations.add(0, 0, cooldown_d * 1.9);
        }
        ...
        RenderUtil.renderModel(ExpandedModelComponents.MACHINE_PISTOL_BOLT.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
		...
		// Render the transformed model.
        ExpandedModelComponents magModel = ExpandedModelComponents.MACHINE_PISTOL_MAGAZINE;
        try {
            ...
            if(!magStack.isEmpty())
            {
	            if (magStack.getItem().builtInRegistryHolder().key().location().getPath().equals("light_magazine"))
		    		magModel = ExpandedModelComponents.MACHINE_PISTOL_LIGHT_MAG;
	            else
	            if (magStack.getItem().builtInRegistryHolder().key().location().getPath().equals("extended_magazine"))
			    	magModel = ExpandedModelComponents.MACHINE_PISTOL_EXTENDED_MAG;
            }
		}
