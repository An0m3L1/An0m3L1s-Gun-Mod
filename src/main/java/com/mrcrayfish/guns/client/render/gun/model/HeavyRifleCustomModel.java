 ...
 */
public class HeavyRifleCustomModel implements IOverrideModel
{
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        BakedModel bakedModel = ExpandedModelComponents.HEAVY_RIFLE_BASE.getModel();
        
        // Render the BakedModel we selected.
        Minecraft.getInstance().getItemRenderer().render(stack, ItemTransforms.TransformType.NONE, false, poseStack, buffer, light, overlay, GunModel.wrap(bakedModel));


        // Render the stock model. This model might be interchangeable via NBT in the future.
    	BakedModel stockModel = ExpandedModelComponents.HEAVY_RIFLE_STOCK_0.getModel();
        /*if (getVariant(stack, "StockVariant") == 1)
        stockModel = ExpandedModelComponents.HEAVY_RIFLE_STOCK_1.getModel();*/
        RenderUtil.renderModel(stockModel, transformType, null, stack, parent, poseStack, buffer, light, overlay);
		...
		{
            RenderUtil.renderModel(ExpandedModelComponents.HEAVY_RIFLE_SIGHTS.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
		}
        ...
        {
            float cooldownDivider = 1.0F*Math.max((float) gun.getGeneral().getRate()/4.5F,1);
            ...
            boltTranslations = boltTranslations.add(0, 0, Easings.EASE_OUT_SIN.apply(cooldown_d) * 3);
        }
        ...
        RenderUtil.renderModel(ExpandedModelComponents.HEAVY_RIFLE_BOLT.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        ...
        /*try {
            ...
            if(!magStack.isEmpty())
            {
	            if (magStack.getItem().builtInRegistryHolder().key().location().getPath().equals("light_magazine"))
		    		magModel = ExpandedModelComponents.HEAVY_RIFLE_LIGHT_MAG;
	            else
