 ...
 * attachment, it will be colored automatically by the color of the weapon if the item does not explicitly
 * have a color set.
 * <p>
 * Author: MrCrayfish
 */
...
{
	/**
	 * Gets whether this item can be colored
	 *
	 * @param stack
	 * 		the ItemStack of the colored item
	 *
	...
	/**
	 * Gets whether this item has a color applied
	 *
	 * @param stack
	 * 		the ItemStack of the colored item
	 *
	 ...
	 * 		the ItemStack of the colored item
	 * @param color
	 * 		the color in rgba integer format
	 */
	...
	static ItemStack dye(ItemStack stack, List<DyeItem> dyes)
	{
		...
		if(IColored.isDyeable(stack))
		{
			...
			{
				float[] colorComponents = dyeitem.getDyeColor()
		...
		{
			final int finalColor = getFinalColor(combinedColors, colorCount, (float) maxColor);
			coloredItem.setColor(resultStack, finalColor);
			return resultStack;
		}
	}
	
	private static int getFinalColor(int[] combinedColors, int colorCount, float maxColor)
	{
		int red = combinedColors[0] / colorCount;
		int green = combinedColors[1] / colorCount;
		int blue = combinedColors[2] / colorCount;
		float averageColor = maxColor / (float) colorCount;
