...
public class SpecialHitMarker extends Crosshair
{
	...
	public void render(Minecraft mc, PoseStack stack, int windowWidth, int windowHeight, float partialTicks)
	{
		...
		float size = 9.0F;
		float hitMarkerProgress = GunRenderingHandler.get()
				.getHitMarkerProgress(partialTicks);
		boolean crit = GunRenderingHandler.get()
				.getHitMarkerCrit();
		float scale = 1.5F + (hitMarkerProgress);
		...
		boolean useAltHitMarker = (Objects.equals(Objects.requireNonNull(CrosshairHandler.get()
						.getCurrentCrosshair())
				.getIDString(), GunMod.MOD_ID + ":hit_marker"));
