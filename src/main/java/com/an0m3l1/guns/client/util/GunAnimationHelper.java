...
import com.an0m3l1.guns.client.AnimationLoader;
import com.an0m3l1.guns.client.AnimationMetaLoader;
...
public final class GunAnimationHelper
{
	...
	public static String getSmartAnimationType(ItemStack weapon, Player player, float partialTicks)
	{
		...
		double reloadTransitionProgress = ReloadHandler.get()
				.getReloadProgress(partialTicks);
		int weaponSwitchTick = ShootingHandler.get()
				.getWeaponSwitchTick();
		if(reloadTransitionProgress > 0.0 && hasAnimation("reload", weapon))
		{
			if(reloadTransitionProgress < 1.0)
			{
				float delta = GunRenderingHandler.get()
						.getReloadDeltaTime(weapon);
				if(hasAnimation("reloadStart", weapon) && ReloadHandler.get()
						.getReloading(player) && delta < 0.4F && ReloadHandler.get()
						.doReloadStartAnimation())
				{
					return "reloadStart";
				}
				else if(hasAnimation("reloadEnd", weapon) && !ReloadHandler.get()
						.getReloading(player) && ReloadHandler.get()
						.doReloadFinishAnimation())
				{
					return "reloadEnd";
				}
			}
		...
		else if(reloadTransitionProgress <= 0.0)
		{
			...
			{
				float cooldown = tracker.getCooldownPercent(weapon.getItem(), Minecraft.getInstance()
						.getFrameTime());
			...
			}
			{
				...
				int lastFiretick = GunRenderingHandler.get()
						.getLastFireTick();
	...
	public static Vec3 getSpecificAnimationTrans(String animType, ItemStack weapon, Player player, float partialTicks, String component)
	{
		...
		{
			float reloadTransitionProgress = ReloadHandler.get()
					.getReloadProgress(partialTicks);
	...
	public static float getSpecificAnimationProgress(String animType, ItemStack weapon, Player player, float partialTicks)
	{
		...
		if(animType.equals("reload") && hasAnimation("reload", weapon))
		{
			return ((GunItem) (weapon.getItem())).getModifiedGun(weapon)
					.getGeneral()
					.usesMagReload() ? GunRenderingHandler.get()
					.getReloadDeltaTime(weapon) : GunRenderingHandler.get()
					.getReloadCycleProgress(weapon);
		}
		if(animType.equals("fire") && hasAnimation("fire", weapon))
		{
			if(getAnimationBoolean(animType, weapKey, "syncToCooldown", true))
			{
				...
				if(maxRate > 1)
				{
					float cooldownDivider = Math.max(fireRate / maxRate, 1);
	...
	public static Vec3 getAnimationTrans(String animationType, ItemStack weapon, float progress, String component)
	{
		...
		if(component.equals("viewModel") /*|| component.equals("gunModel")*/)
		{
			...
			boolean rightHand = mc.options.mainHand()
					.get()
					.equals(HumanoidArm.RIGHT);
	...
	public static double getAnimationValue(String animationType, ItemStack weapon, float progress, String component, String valueName, double returnVal)
	{
		...
		return getAnimationValue(animationType, weapKey, component, priorFrame, valueName, returnVal);
	}
	...
	{
		boolean magReload = ReloadHandler.get()
				.isDoMagReload();
		boolean emptyReload = ReloadHandler.get()
				.isReloadFromEmpty();
		String animSuffix = "";
		if(animationType.contains("reload") && (magReload || emptyReload))
		{
			...
			else if(magReload && hasAnimation(animationType + "_Mag", weapKey))
			{
				animSuffix = "_Mag";
			}
			else if(emptyReload && hasAnimation(animationType + "_Empty", weapKey))
			{
				animSuffix = "_Empty";
			}
		}
	...
	static ResourceLocation lookForParentAnimation(String animationType, ResourceLocation weapKey)
	{
		...
		if(animObject.has("parent", DataType.STRING))
		{
			...
			assert parent != null;
			String[] splitString = parent.asString()
					.split(":");
			if(doParentMessage1)
			{
				GunMod.LOGGER.info("Animation System (1): Successfully detected the parent object of {}. Parent object is {}", weapKey, new ResourceLocation(splitString[0], splitString[1]).toString());
		...
		else
		{
			...
			if(animObject.has("parent", DataType.STRING))
			{
				...
				if(doParentMessage2)
				{
					GunMod.LOGGER.info("Animation System (2): Successfully detected the parent object of {}. Parent object is {}", weapKey, new ResourceLocation(splitString[0], splitString[1]).toString());
	...
	static Vec3 getAnimationArray(String animationType, ResourceLocation weapKey, String component, String transform)
	{
		...
		if(transformObject.has(transform, DataType.ARRAY))
		{
			...
			{
				return PropertyHelper.arrayToVec3(transformArray, Vec3.ZERO);
			}
		}
	...
	public static float getAnimationSoundParamFloat(String animationType, ItemStack weapon, int soundIntID, String parameter, float defaultValue)
	{
		...
		{
			DataNumber audioThresholdData = audioObject.getDataNumber(parameter);
			if(audioThresholdData != null)
			{
				return audioThresholdData.asFloat();
			}
		}
		return defaultValue;
	}
	...
	private static DataObject getCustomData(ResourceLocation location)
	{
		...
		if(useLegacyLoader)
		{
			return AnimationMetaLoader.getInstance()
					.getData(location);
		}
		return AnimationLoader.getInstance()
				.getData(location);
	}
}